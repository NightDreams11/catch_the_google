...
function _doesPositionMatchWithPlayer1Position(position) {
  return (
...
function _doesPositionMatchWithPlayer2Position(position) {
  return (
...
function _doesPositionMatchWithGooglePosition(position) {
  return (
...
export async function movePlayer(playerNumber, direction) {
  if (_state.gameStatus !== GAME_STATUSES.IN_PROGRESS) {
    console.wart("You can move player only when game is in progress")
    return
  }
  const playerIndex = _getPlayerIndexByNumber(playerNumber)
